...
let _Papa; // This will now be the global Papa object
 ...
 * @param {object} Papa - Papa Parse library instance (now passed as global).
...
export function initAdmin(db, auth, appId, currentUserId, serverTimestamp, Papa, showConfirmModal, collectionFn, queryFn, whereFn, addDocFn, getDocsFn, docFn, updateDocFn, deleteDocFn) {
    ...
    _Papa = Papa; // Assign the global Papa object
    ...
    // Ensure _db is not null before trying to create collection reference
    if (_db) {
        termsCollectionRef = _collection(_db, `artifacts/${_appId}/public/data/terms`);
    } else {
        console.error("Firestore DB instance is null. Cannot create termsCollectionRef.");
        // Handle gracefully, perhaps disable import/export features if DB is not available
        const importCsvBtn = document.getElementById('import-csv-btn');
        const clearDatabaseBtn = document.getElementById('clear-database-btn');
        if (importCsvBtn) importCsvBtn.disabled = true;
        if (clearDatabaseBtn) clearDatabaseBtn.disabled = true;
    }
    ...
    // Only attach if _db is available, otherwise these operations won't work
    if (importCsvBtn && _db) importCsvBtn.addEventListener('click', handleImportCsv);
    if (clearDatabaseBtn && _db) clearDatabaseBtn.addEventListener('click', handleClearDatabase);
...
async function displayTermsMatrix() {
    ...
    // Ensure Firestore is initialized before attempting to fetch
    if (!_db || !termsCollectionRef || !_getDocs) {
        console.warn("Firestore not fully initialized for displayTermsMatrix. Cannot fetch terms.");
        termsMatrixBody.innerHTML = '<tr><td colspan="3" class="text-center py-4 text-red-400">Database not available.</td></tr>';
        return;
    }
...
async function handleSaveTerm(event) {
    ...
    if (!_db || !termsCollectionRef || !_addDoc || !_getDocs || !_updateDoc || !_doc || !_where || !_query || !_serverTimestamp) {
        statusMessage.textContent = 'Error: Database functions not available. Please ensure Firebase is initialized and authenticated.';
    ...
    try {
        ...
        if (termId) {
            ...
            // Add new term
            await _addDoc(termsCollectionRef, termDataToSave); // Changed dataToSave to termDataToSave
...
async function processCsvFile(file, fileName, importStatusMessage, csvFileInputElement) {
    ...
    if (typeof _Papa === 'undefined') {
        importStatusMessage.textContent = 'Error: PapaParse library not loaded.';
...
async function handleImportCsv() {
    console.log("handleImportCsv function called."); // Added console log
    const csvFileInput = document.getElementById('csv-file-input');
